...
public class TreeHeight {
	...
	public static int heightDFS(Node node) {
		...
		while (!stack_node.isEmpty()) {
			Node crrent_node = stack_node.pop();
			int current_depth = stack_depth.pop(); 
			maxHeight = Math.max(current_depth, maxHeight);
			
			if (crrent_node.left != null) {
				stack_node.push(crrent_node.left);
				stack_depth.push(current_depth+1);
			}
			if (crrent_node.right != null) {
				stack_node.push(crrent_node.right);
				stack_depth.push(current_depth+1);
			}
		}
...
>>>>>>> 749de5837197786e6041ebb206efc5d3a8490af4
}